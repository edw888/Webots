...
/* include headers, Importing libraries*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
/* Webots libraries */
...
/* Device stuff, Constants and variables defined. 
Arrays declared to store device tags, values and names*/
/* Distance sensors */
...
/* Motor control parameters defined. Constants for controlling both motors. 
Array for speed used to store desired speeds for both */
...
/* Breitenberg stuff
2D array weights to store weights for Braitenberg algo. Determine influence of each sensor
on the robot motor speed. Offsets are added to the motor speeds */
...
/* Helper fuctions to facilitate control of robot */
/* Retrieves time step */
...
/* Wait for specified seconds using step() */
...
/* Initialize the sensors, led, ground sensors and motors */
...
/* resets motor speeds and LED to default values */
...
/* Retrieves values from distance and ground sensors */
static void get_sensor_input() {
  int i;
  for (i = 0; i < DISTANCE_SENSORS_NUMBER; i++) {
    distance_sensors_values[i] = getValues(distance_sensors[i]);
  ...
  for (i = 0; i < GROUND_SENSORS_NUMBER; i++) {
    if (ground_sensors[i])
      ground_sensors_values[i] = getValues(ground_sensors[i]);
  }
}
/* Checks if ground sensors detect cliff (bellow certain threshold) */
...
/* Sets LED states and motor speeds according to calculated values */
...
/* Alternates LEDs to create blinking effect */
static void blink_leds() {
  static int counter = 0;
  counter++;
  leds_values[(counter / 10) % LEDS_NUMBER] = true;
}
/* Calculates motor speeds based on sensor values using Braitenberg algo */
...
/* Sets the motor speeds to make robot move backwards for short duration */
static void go_backwards() {
  wb_motor_set_velocity(left_motor, -MAX_SPEED);
  wb_motor_set_velocity(right_motor, -MAX_SPEED);
  passive_wait(0.2);
}
/* Sets motor speeds to make robot turn left for short duration  */
static void turn_left() {
  wb_motor_set_velocity(left_motor, -MAX_SPEED);
  wb_motor_set_velocity(right_motor, MAX_SPEED);
  passive_wait(0.2);
}
/* Defines main(), entry point of the program.
Initializes robot, devices and enters loop that does steps:
1 Resets actuator values (motor speeds and LED states)
2 Retrieve sensor input (distance sensor values and ground sensor values).
3 Blink the LEDs.
4 Check if a cliff is detected.
5 If a cliff is detected, make the robot go backwards and turn left.
6 If no cliff is detected, calculate motor speeds using the Braitenberg algorithm.
7 Set the actuator values (LED states and motor speeds).
8 Step the simulation. */
...
/* Program will run indefinitely until terminated externally or sim stopped  */
  return EXIT_SUCCESS;
